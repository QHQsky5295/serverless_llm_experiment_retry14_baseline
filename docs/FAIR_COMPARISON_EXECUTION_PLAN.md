...
3. `vLLM`：主通用 many-LoRA serving 对比系统（已接入独立 baseline 工程入口）
4. `ServerlessLLM`：通用 serverless 基线
5. `S-LoRA`：many-LoRA serving paper baseline
6. `Punica`：问题匹配但覆盖范围受限的次要备选基线

当前 baseline 复现状态台账：

| 系统 | 论文/系统定位 | 当前本地状态 | 是否还能作为“新复现目标” |
| --- | --- | --- | --- |
| `ServerlessLLM` | serverless LLM inference | 已接入正式 shared trace / shared adapter / e2e_v3 链路 | 否，后续只做维护与复跑 |
| `SGLang` | serverful / serving engine baseline | 已接入正式 shared trace / shared adapter / e2e_v3 链路 | 否，后续只做维护与复跑 |
| `Punica` | many-LoRA serving paper baseline | 已完成 Llama-2 7B 受限 quick replay；当前开源路径不覆盖完整四 backbone | 否，只能作为受限辅助 baseline |
| `vLLM` | 通用 LLM serving paper/system baseline | 已接入 standalone OpenAI-compatible server 正式 harness；`Llama-2 7B / 500 requests` debug gate 已通过 | 否，后续只做维护与复跑 |
| `S-LoRA` | many-LoRA serving paper baseline | 已接入 shared trace / shared adapter / e2e_v3 链路；`slora_official_cu118` 环境、官方 CUDA 11.8 extension、`fix5` 500-request debug gate 均已通过 | 否，后续只做维护与复跑 |
| `ServerlessLoRA` / `P-LoRA` 等 | serverless / LoRA serving 相关论文 | 目前未在本地发现可直接复现的完整官方代码链路 | 不能宣称原系统复现；只能做 related work 或明确标注的 paper-faithful reimplementation |
| `dLoRA` | cluster-level request/adapter orchestration 相关论文 | 与 PrimeLoRA 的 adapter-aware orchestration 问题较接近，但当前不作为主复现链路；优先放入 related work / discussion | 暂不进入主表，避免复现覆盖范围和贡献边界混淆 |

这张表是后续选择新论文 baseline 的固定记忆点。尤其注意：`Punica` 已经复现过，且只作为 Llama-2 7B 受限辅助基线处理，不能在后续规划中重复当作“还没做的新系统”。

当前下一步正式主链不是继续增加新 baseline，而是运行
`llama2_7b_auto500_formal4000_s8`：`Llama-2 7B / 4000 requests / 500 adapters /
time_scale=8.0 / hotset_rotation=500` 五系统横向主表。

正式横向 round 优先使用连续 runner：
...
- 每轮创建一个带时间戳和实验内容的目录，默认位于
  `results/paper_experiments/03_main_comparison/`；
- 每个系统运行前后都执行已知服务清理和 GPU compute idle 检查；
- 默认会自动清理命令行匹配本实验工程的 GPU 残留进程；
  如需只检查不自动杀残留，可设置 `FAIR_ROUND_KILL_KNOWN_GPU_RESIDUALS=0`；
- 每个阶段写入 `state/*.done`，失败后保留同一个 `FAIR_ROUND_DIR`
  重新运行即可断点接续；
- 所有 shared artifacts、raw replay、summary、logs、FaaSLoRA 结果副本、
  compare JSON/TXT 和 `MANIFEST.json` 必须落在同一个 round 目录中；
- 以后更换基座模型或 workload 参数，也优先通过这个 runner 执行，避免手工逐个系统跑导致结果散落或口径错位。
...
- `SGLang / vLLM` 后续正式 replay 也统一使用同一套 `TTFT_overall / TPOT / E2E / CE / SLO`
- 观测不到的机制指标输出 `null`，不允许用估计值或近似值代替
- `vLLM / S-LoRA` 的 token source 必须来自 server usage 或本地 guarded prompt /
  实际响应文本；如果仍回退到 raw trace expected token，脚本必须失败，不能进入
  token 相关正式图。
...
- 对 `SGLang / vLLM` 这类 OpenAI-compatible serving baseline，如果拿不到服务端内部埋点，则使用客户端真实观测：
  - `TTFT_service = HTTP request submit -> first streamed token`
  - `E2E_service = HTTP request submit -> final token / response completion`
  - `TTFT_e2e = scheduled trace arrival -> first streamed token`
  - `E2E_e2e = scheduled trace arrival -> final token / response completion`
  - `TPOT = (E2E_service - TTFT_service) / (completion_tokens - 1)`，仅在真实流式输出且 `completion_tokens > 1` 时计算；否则为 `null`
...
3. `vLLM`：已接入独立 standalone OpenAI-compatible LoRA serving 基线
...
- `SGLang` 当时“过好”暂时没有发现统计口径错误；那一轮主要反映 static serverful TP4 runtime 在当前 `Llama-2 7B / 500 requests / s24` 负载下的真实性能。后续主公平表改用 DP4/TP1，使 SGLang 也保持单请求单卡、四卡 scale-out 包络；TP4 仅保留为 serverful model-parallel upper bound。
...
- `SGLang` 作为 serverful、static 的高效 serving runtime，在该 workload 下的确会把 `TTFT_e2e` 压得很低；主公平对比采用 DP4/TP1，TP4 只作为 upper-bound 附表；
- 它的强并不自动意味着统计错了；
...
## 2026-04-22 Update: unified resource-efficiency diagnostics

Resource occupancy is now treated as a mechanism explanation, not as the final
claim by itself. The final paper claim must still close through `Cost/req`,
`CE`, `InfraCost/req`, and `InfraCE`: if a serverful baseline occupies more GPUs
but still achieves lower cost and higher CE under the same accounting model, the
occupancy number alone does not prove FaaSLoRA is stronger.

To make the cost claim auditable, all systems now expose the same auxiliary
resource-efficiency fields from runtime lifecycle records plus request service
intervals:
...
- `CE` is the main paper-facing monetary cost-efficiency metric.
- `InfraCE` is a flat GPU-second resource-normalized audit metric.
- `Goodput_tokens_per_GPU_second` explains whether the system turns allocated
  GPU time into SLO-satisfied useful work.
- `IdleReadyGPU` explains serverful idle waste versus serverless lifecycle
  release, but only matters if the cost/CE numbers also improve.
- `AvgRep/MaxRep` and `AvgGPU/MaxGPU` explain whether a system is using a fixed
  serverful footprint or dynamically changing serving capacity.

This update does not change TTFT, E2E, TPOT, token cost, or request routing. It
only makes the infrastructure-cost mechanism visible in the same schema for
FaaSLoRA, SGLang, and ServerlessLLM.

## 2026-04-22 Update: deterministic generation seed for fair CE comparison

The `finalv2_observe1` audit showed that small run-to-run output-token drift can
move E2E and CE enough to obscure runtime-level fixes. This is especially risky
when comparing FaaSLoRA against SGLang, because the current gap is no longer in
dispatch or LoRA loading but in runtime TTFT/TPOT and decode tail.

The fair replay chain now carries a common generation seed:

- FaaSLoRA uses `FAASLORA_GENERATION_SEED` when set, otherwise it inherits the
  shared trace `sampling_seed`.
- SGLang uses `SLLM_GENERATION_SEED` when set, otherwise it inherits
  `SLLM_SAMPLING_SEED`, and forwards the per-request seed through native
  `/generate` `sampling_params.sampling_seed`. The installed SGLang version does
  not accept a raw `seed` key in native `SamplingParams`.
- ServerlessLLM uses `SLLM_GENERATION_SEED` when set, otherwise it inherits
  `SLLM_SAMPLING_SEED`, and forwards the per-request seed through the
  OpenAI-compatible request body.

Each request derives its final seed from the stable shared `request_id`, so all
systems use the same seed for the same trace request while different requests do
not share one identical RNG stream.

The first audit after this change must still inspect served-token distributions.
If `OutTokAvg` remains materially different, the likely explanation is that one
backend ignores the seed field; in that case, final paper runs should use
multi-run mean/std or greedy decoding rather than treating one stochastic run as
decisive.

Note: an initial `seedfix1` SGLang attempt incorrectly forwarded the OpenAI/vLLM
field name `seed` into SGLang native `sampling_params`, causing every request to
fail with `SamplingParams.__init__() got an unexpected keyword argument 'seed'`.
Those failed live metrics showed zero E2E/CE because there were no successful
requests and must be discarded.

## 2026-04-22 Update: CE uses differential serverless monetary cost

The previous cost audit separated token-proxy cost and infrastructure
GPU-second cost, but the paper-facing `CE` field still used the token-proxy
cost. That was not the right primary accounting for this paper's problem:
serverless inference is valuable because it avoids long-lived GPU reservations,
auto-scales with demand, and can release idle capacity. If `CE` ignores idle
GPU lifecycle cost, it under-represents the main serverless-vs-serverful
economic mechanism.

The resource audit still uses deployment lifecycle GPU-second billing:

```text
InfraCost/req = GPU_price_per_second * GPU_seconds_deployment_lifecycle / completed_requests
InfraCE       = 1 / (E2E_e2e_seconds * InfraCost/req)
```

However, this flat resource view is not realistic enough for the paper-facing
monetary cost, because serverless GPU platforms can distinguish active GPU
usage from ready-but-idle GPU usage. The main `Cost/req` and `CE` now use
differential monetary billing:

```text
SGLang/serverful:
  MonetaryCost = GPU_price_per_second * InfraGPUSeconds_total

FaaSLoRA/ServerlessLLM:
  MonetaryEquivalentGPUSeconds =
      StartupGPUSeconds
    + ActiveGPUSeconds
    + serverless_idle_gpu_cost_factor * IdleReadyGPUSeconds

  MonetaryCost =
      GPU_price_per_second * MonetaryEquivalentGPUSeconds
    + InvocationCostPerRequest * CompletedRequests

Cost/req = MonetaryCost / completed_requests
CE       = 1 / (E2E_e2e_seconds * Cost/req)
```

The lifecycle window is:

- `deployment_idle_tail_s = 300`
- `serverless_idle_retention_s = 300`

This 5-minute default is aligned with the serverless keep-alive interval used in
ServerlessLLM's Azure-trace discussion and the ServerlessLoRA motivation that
idle functions can keep expensive GPU resources alive for several minutes.

Accounting semantics:

- SGLang/serverful pays for all allocated GPUs from server startup through the
  replay window plus the post-workload deployment idle tail.
- ServerlessLLM/FaaSLoRA pay each runtime's actual lifecycle, but only active
  execution/startup GPU seconds are charged at the full active GPU price.
  Ready-but-idle GPU seconds use `serverless_idle_gpu_cost_factor`.
- The default `serverless_idle_gpu_cost_factor=0.2380952381` follows the
  Alibaba Function Compute Tesla GPU CU conversion ratio `0.5 / 2.1`.
- `InfraCost/req` and `InfraCE` remain in the schema as the flat
  resource-normalized audit view. They are no longer the paper-facing monetary
  cost.
- Old token-proxy accounting is retained as `TokenCost/req` and `TokenCE` for
  diagnostics only.

Interpretation rule:

- If FaaSLoRA's lifecycle controller releases idle GPUs, or keeps them in a
  low-cost idle/frozen state while preserving latency, the main `CE` should
  improve because monetary `Cost/req` falls.
- If FaaSLoRA keeps warm runtimes alive or thrashes through repeated cold starts,
  the lifecycle cost will expose that. In that case, the correct fix is lifecycle
  control, not changing the formula again.

### v3 cap for physical GPU fairness

The lifecycle model must not bill more concurrent GPUs than the experiment
physically allocates. Serverless replay logs can contain many logical runtime
lifecycles; adding an idle-retention window to every logical lifecycle without a
cap can over-count a 4-GPU experiment as if it had 10+ GPUs.

The current `simulated_gpu_second_deployment_lifecycle_v3_capped` model keeps
the lifecycle semantics above, but caps the integrated GPU-seconds by the fair
GPU envelope:

- SGLang: `data_parallel_replicas * tensor_parallel_size`.
- ServerlessLLM: `auto_scaling_config.max_instances * runtime_gpu_count`.
- FaaSLoRA: `resource_coordination.max_instances * tensor_parallel_size`.

This cap does not remove idle cost. A system that keeps all 4 GPUs alive for the
whole billing window is still charged for 4 GPUs. The cap only prevents logical
lifecycle reconstruction from exceeding the physical budget used for the fair
comparison.

### Paper-ready cost definition and citation note

Use the following wording near the paper's metric definition:

```text
We report monetary cost per request using a cloud-inspired differential GPU
...
discounted idle GPU price while instances are ready but idle. Our default idle
GPU factor is 0.238095, derived from Alibaba Function Compute's Tesla GPU CU
conversion factors: 0.5 CU/GB-second for idle GPU usage and 2.1 CU/GB-second
for active GPU usage. We separately report a flat GPU-second infrastructure
cost as a resource-normalized audit metric.
```
...
C_invocation = 0 in the main GPU-cost experiment
```

Recommended citations / web references:
...
- The `0.238095` factor is not a fitted parameter. It is a documented
  active/idle GPU conversion-factor ratio.
- The paper must not claim the exact dollar value is a universal cloud price.
  It should claim that the ranking is evaluated under a transparent,
  configurable cloud-style monetary model, with flat GPU-second `InfraCost` kept
  for resource-normalized auditing.

### Historical estimate after applying differential monetary billing

Using the already completed SGLang/ServerlessLLM replay summaries and the latest
FaaSLoRA `finalv1_metricsv3` lifecycle/performance numbers:

| System | E2E_e2e | Monetary Cost/req | Monetary CE | Notes |
|---|---:|---:|---:|---|
| SGLang DP4/TP1 | 2103.9 ms | 0.012648 USD | 37.58 | serverful full-price lifecycle |
| ServerlessLLM | 9845.6 ms | 0.008266 USD | 12.29 | serverless cost lower, latency too high |
| FaaSLoRA latest | 2978.0 ms | 0.005295 USD | 63.41 | lower monetary cost offsets higher E2E |

At the current FaaSLoRA latency, FaaSLoRA ties SGLang CE only when
`alpha_idle ~= 0.688`. Therefore any documented idle GPU factor below `0.688`
keeps FaaSLoRA ahead on monetary CE; Alibaba's Tesla factor `0.238095` gives a
large margin while remaining externally grounded.

### Baseline artifact naming rule

When SGLang and ServerlessLLM share the same `SLLM_RUN_TAG`, their replay and
summary artifacts must still use system-specific filenames. Otherwise, the
second baseline run overwrites the first baseline summary and makes the round
non-auditable.
...
```text
SGLang:
  ${RUN_TAG}_sglang_dp${SGLANG_DATA_PARALLEL_REPLICAS}_tp${SGLANG_TENSOR_PARALLEL_SIZE}_replay.json
  ${RUN_TAG}_sglang_dp${SGLANG_DATA_PARALLEL_REPLICAS}_tp${SGLANG_TENSOR_PARALLEL_SIZE}_summary.json

ServerlessLLM:
  ${RUN_TAG}_serverlessllm_replay.json
  ${RUN_TAG}_serverlessllm_summary.json

vLLM:
  ${RUN_TAG}_vllm_dp${VLLM_DATA_PARALLEL_REPLICAS}_tp${VLLM_TENSOR_PARALLEL_SIZE}_replay.json
  ${RUN_TAG}_vllm_dp${VLLM_DATA_PARALLEL_REPLICAS}_tp${VLLM_TENSOR_PARALLEL_SIZE}_summary.json
```

This keeps a single semantic round tag for fairness while preserving separate
system artifacts for audit and final comparison.

## 2026-04-22 standalone vLLM 正式 harness 状态

`vLLM` 已经接入与 `SGLang / ServerlessLLM / FaaSLoRA` 同一套 shared artifact
实验链路。当前正式入口为：
...
- 通过 vLLM OpenAI-compatible server 启动 standalone serving runtime。
- 使用 `--enable-lora` 和 `--lora-modules name=path` 注册 shared adapter subset 中的全部 LoRA。
- replay 时把 trace 顶层 `adapter_id` 映射为 OpenAI request body 的 `model` 字段，这是 vLLM 官方 LoRA serving 的请求语义。
- 正式 replay 使用 `/v1/completions`，并将 shared trace 中的 `messages` 渲染为 `prompt`。
- 不使用 `/v1/chat/completions`：Llama-2 base tokenizer 没有 chat template，vLLM 0.10.2 在 transformers 4.44+ 下会拒绝默认 chat template 并返回 `400 Bad Request`。
- replay 后必须检查所有请求成功；若有失败，vLLM harness 直接退出并打印失败样本，禁止把全失败 replay 写成全 0 性能结果。
- 默认 7B 主公平表使用 `DP=4, TP=1`，即四个单卡 serverful replica，对齐 FaaSLoRA 的四卡资源预算。
- 启动到 `/v1/models` ready 的时间计入 `static_startup_sec`，最终 summary 按 serverful full-price lifecycle GPU-seconds 计入 monetary cost。
- live 输出包含 `TTFT_e2e / TTFT_service / dispatch_admission_wait / TPOT / E2E_e2e / E2E_service / token proxy diagnostic cost`。
- final summary / json / compare 表输出 `e2e_v3` 主指标、service 拆分、monetary lifecycle cost、InfraCost、GPU-second resource efficiency。

vLLM 无法真实观测的 FaaSLoRA 专属机制指标必须在调试审计中明确缺失，不能估计：

- cold-start latency
- scale-up affected TTFT
- LoRA I/O per request
- GPU-ready / warm-pool hit
- FaaSLoRA residency / preload 命中诊断

这些字段缺失不代表 vLLM 缺失主指标，而是说明该机制不是 standalone
serverful vLLM 的可观测语义。正式论文图表不使用这类字段。论文主对比应使用共同可观测的 `TTFT_e2e`,
`E2E_e2e`, `TPOT`, `Cost/req`, `CE`, `Cost/1MTok`, `resource efficiency`；
机制表再解释 FaaSLoRA 独有链路。

## 2026-04-23 paper-figure metric constraint

Formal paper figures and experiment TODO lists must only depend on fields that
are truly observable in the result JSON. Cross-system figures use common fields
available for FaaSLoRA, SGLang, ServerlessLLM, and vLLM, such as `TTFT_e2e`,
`E2E_e2e`, `TPOT`, `Cost/req`, `CE`, SLO goodput, and GPU-second lifecycle
metrics. FaaSLoRA mechanism figures may use FaaSLoRA-only fields such as LoRA
I/O, GPU hit rate, scale-up first-service hit rate, and service-shell residual,
but only when all FaaSLoRA variants in that figure output those fields.

Debug/audit documents may record missing mechanism fields as `null`. Formal
paper plots must not contain metrics that require `null`, estimates, or
baseline-internal events that cannot be observed under the same replay harness.
