...
2. 是否有公开代码，并且能在本机环境构建或至少完成可信适配检查；
...
- 对没有代码、运行时不匹配、或者需要改核心语义的无服务器大模型论文，放入适配检查/附录/相关工作表，而不是静默忽略。
...
| Loquetier | 真实 adapter 适配检查通过，但 3B/500 adapter 预检 OOM | 24GB 显存包络不足，加上缺上游分片 adapter 放置 | 普通软件适配不够 | 要做多 GPU/分片 adapter placement 才能正式跑，属于核心实现。 |
...
| HydraServe/ParaServe | 控制面和源码适配检查已做 | Kubernetes/GPU pod 运行环境，以及 per-request adapter identity 语义 | K8s 是平台问题；adapter identity 是核心语义问题 | 只搭环境还不够；加入动态 adapter 请求语义会改核心。 |
| AIBrix | 组件和 runtime sidecar 适配检查已做 | Kubernetes/Docker/GPU pod 权限 | 理论上可在有权限环境中继续 | 当前本机权限不够；裸 vLLM+sidecar 不能声称是 AIBrix。 |
...
| TIDAL / ServerlessLoRA / Predictive-LoRA / PipeBoost / SLINFER / Tangram / MoEless | 论文调查 | 主要缺公开官方代码；部分还依赖不同运行时或负载形态 | 自己实现不算复现 | 只能相关工作或未来代码公开后再适配检查。 |
...
| LoRAX / TGI Multi-LoRA | 工程/adapter-serving 候选 | 非 serverless cold-start 论文系统 | 可以软件适配检查 | 可作为 multi-LoRA serving baseline，不是无服务器论文系统主表复现。 |
...
| dLoRA | 完成本地构建/导入、现代 Ray/CUDA 兼容、真实 PEFT loader、Llama-3.2 GQA 处理、3B 官方 `migration_type=3` 完整 replay，以及多轮 7B 内存拓扑适配检查。 | 7B 无法达到 HTTP readiness。最终 G1/TP4 适配检查在 `gpu_capacity=1`、`gpu_memory_utilization=0.99` 下，500 个 adapter 物化并加载后仍出现 `# GPU blocks: 0`。 | 只有改核心 cache 分配、adapter 放置、量化、rank 布局、模型/adapter 内存管理才可能继续。这会变成非忠实 dLoRA。 |
| Loquetier | 建立隔离环境，处理 Python/PEFT/CUDA 12.1 兼容，完成 3B/7B 真实 PEFT adapter 适配检查，并从真实 tensor 处理混合 rank。 | 3B/500 adapter 预检在 24GB RTX 3090 上物化 adapter 权重时 OOM。 | 正式运行需要上游兼容的多 GPU 或分片 adapter 放置。我们自己实现会改变 Loquetier 核心。 |
...
| HydraServe | 完成控制面导入、平台适配检查、内嵌 vLLM LoRA 接口审计。 | 需要 Kubernetes GPU 部署；请求路径不能保留我们 LoRA 负载所需的 per-request adapter identity。 | 给 scheduler/request 语义添加 adapter identity 属于核心语义改动。 |
...
| Preble | 目前只做在线/源码初筛。 | 它是 prefix-cache 路由系统，不是无服务器 LoRA 生命周期系统；adapter identity 和真实远程 LoRA 语义未建立。 | 可作为附录适配检查候选，但要先做源码级 adapter identity 审计；不是第一优先级正式候选。 |
...
|---|---:|---|---|---|---|---|
| LoRAX / Predibase | 活跃开源，最新 release 为 2025-01 左右 | `github.com/predibase/lorax`，Apache-2.0，提供生产 Docker/Helm，支持从 filesystem/HF/Predibase 动态加载 adapter，OpenAI-compatible API | 是 | 强：multi-LoRA server、异构 batching、adapter exchange scheduling | 可能是 4x3090 上最好的实用适配检查候选，但不是 serverless cold-start；必须先测 Llama-2 7B 和 Llama-3.2 3B 本地 PEFT 兼容与 500 adapter 内存包络 | 高优先级新增适配检查；如果论文接受非 serverless 的 multi-LoRA serving baseline，可进附录或额外表 |
| HuggingFace TGI Multi-LoRA | 2024 之后的生产特性 | TGI 支持 multi-LoRA serving；开源，但可能需要 Docker/Rust/CUDA 构建 | 是 | 足够强，可做 per-request adapter serving | 4x3090 可能可行，但与 LoRAX/TGI 技术线重叠，运维更重 | LoRAX 后的第二适配检查候选 |
...
| llm-d | 到 2026 为活跃开源/CNCF sandbox | `github.com/llm-d/llm-d`，Kubernetes/vLLM 平台，文档提到 scale-to-zero 和 cache-aware LoRA routing | 是 | 可通过 vLLM LoRA routing 使用 | 需要 Kubernetes/Gateway/KEDA 等重平台；当前权限与 AIBrix/HydraServe 阻塞类似 | 未来平台适配检查，需 Kubernetes 权限 |
| NVIDIA Dynamo | 活跃开源 | `github.com/ai-dynamo/dynamo`，文档提到动态 LoRA loading/routing | 是 | 潜在有用 | 数据中心级分布式 stack，容器/runtime 依赖重；也不是论文基线，4x3090 本地复现实用性低 | 未来平台适配检查，不作为当前正式基线 |
...
| SLINFER / LLM-Mesh | HPCA 2026 | 找到论文，未定位公开官方代码 | 是，偏小/中模型 serverless inference | 一般多模型资源共享，不专门做 adapter | 评估环境是 CPU/A100 异构，无代码 | 相关工作/未来适配检查 |
...
| ParaServe/HydraServe | NSDI 2026 | 已作为 HydraServe 做过代码适配检查 | 是 | 内嵌 vLLM 有静态 LoRA 参数，但没有 per-request adapter identity | 已被 K8s 和请求语义阻塞 | 已闭为附录/适配检查证据 |
| LambdaScale / FaaScale | 2025 arXiv | 已做代码适配检查 | 是 | 没有可直接复用的 LoRA workload path | 缺 RDMA/IB 栈 | 已闭为附录/适配检查证据 |
...
如果每个新增对比都必须是“论文系统”，下一步就主要是文档/源码适配检查，而不是直接性能 replay：TIDAL、SLINFER、Tangram、ServerlessLoRA、DeepServe、MoEless 等只有在公开官方代码出现后，才适合进入正式复现队列。
...
但 LoRAX 必须谨慎定位：它是 multi-LoRA 服务基线，不是无服务器冷启动基线。公平适配检查应先检查构建/导入、Llama-3.2 3B 真实 PEFT adapter smoke、Llama-2 7B 真实 PEFT adapter smoke、小规模闭口 trace replay，再考虑 4000 请求真实远程运行。

当前优先级如果论文允许非论文工程系统：

1. Ray Serve + vLLM autoscaling/scale-to-zero 适配检查；
2. LoRAX 适配检查；
3. TGI Multi-LoRA 适配检查。

当前优先级如果新增基线必须是论文系统：

1. `ServerlessLLM-new` 已经完成；
2. HydraServe、Medusa、FaaScale、Sarathi 已经适配检查并被正式 replay 阻塞；
