...
## 2026-04-13 更新：本次同步应冻结 4 模型正式基线，并把系统主链先收口
...
- 当前正式论文家族已固定为：
  ...
  - `Llama-2 13B TP=2`
- 当前必须纳入本次同步的新结论：
  - `CE = 1 / (avg_e2e_sec * avg_cost_usd)` 已作为正式综合主指标固定
  - `TTFT_overall` 已固定为 TTFT 家族主指标
  - `TTFT_comp / TTFT_warm` 保留为论文对齐与机制诊断指标
  - `QPR_TOKPS_TTFT_legacy` 仅保留记录，不再主展示
  - 当前四个正式模型结果已经足够支撑“先收口、待跨论文对比后再调”的工程决策
  - `Llama-2 13B` 当前虽弱于 `Qwen 14B`，但证据更指向模型 serving envelope 差异，而不是系统主链再有明显漏洞
- 当前必须纳入本次同步的代码与文档：
  - `configs/experiments.yaml`
  - `configs/generated/publicmix/llama2_*`
  - `scripts/setup_llama2_family.sh`
  - `scripts/rebuild_llama2_13b_publicmix.sh`
  - `scripts/run_all_experiments.py`
  - `tests/test_basic_smoke.py`
  - `docs/*.md`
- 当前正式收口判断：
  - 系统主链先 soft-close
  - 后续只有在跨论文系统对比明确暴露新的系统级短板时，才允许重开主链优化
  - 不再为了单模型单指标继续补丁式调参

## 2026-04-12 更新：本次同步应冻结 7B checkpoint、14B baseline45 与 Mistral 7B baseline3，并把下一步切到 Mistral Nemo TP=2
...
- 当前必须纳入本次同步的新结论：
  ...
  - `Mistral 7B baseline3` 已证明：
    - 问题根因不是控制链损坏，而是历史稳定性保守壳中的 `max_loras=1`
    - 将 `max_loras` 恢复到 `4` 后，`TTFT / Tok/s / QPR / avg_lora_io_ms` 全部大幅回正
    - 因而当前 `continuous_queue_v2` 主线已具备跨模型家族可迁移性，不再只对 Qwen 家族有效
  - 当前 14B 尚未收口的部分已经转到 C2/C3：
    - `TTFT_scaleup_affected_avg_ms = 4060.3`
    - `Cold_start_latency = 67901.1 ms`
    - `warm_pool_hits = 0`
  - 下一条 active 主线应切到 `Mistral Nemo TP=2`
- 当前必须纳入本次同步的代码语义：
  ...
  - `Mistral 7B` 当前已从历史稳定性保守壳中恢复到更合理的 `max_loras=4`
  - `Mistral Nemo TP=2` 的 workload `max_instances` 已与当前 `4 x 3090` 的物理上限对齐到 `2`
- 当前必须纳入本次同步的代码与文档：
  - `scripts/run_all_experiments.py`
  - `tests/test_basic_smoke.py`
  - `docs/*.md`
...
3. 后续 active 主线切到 `Mistral Nemo TP=2`。
4. 如果 `Mistral Nemo` 也复现同样的后续接管冷路径问题，再回到跨模型共通的 C2/C3 主线继续收口。
