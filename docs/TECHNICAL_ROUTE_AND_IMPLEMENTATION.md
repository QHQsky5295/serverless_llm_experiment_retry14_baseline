...
- 完整版默认配置是 `500 LoRA + representative 1000 requests`
- 当前论文主实验使用主模式 `auto500 + 1000 representative requests`

### representative sampling 的当前实现

当前代码已经接入 representative sampling 主路径。它不会简单均匀截断请求，而是尽量保持 full trace 的以下统计特征：
...
因此，当前矩阵 workload 的方法学是：
- representative Azure trace replay
- ShareGPT prompt pool
- configurable `time_scale_factor`
...
系统统一维护三层本地工件驻留状态，并通过远端源仓库补给缺失工件：

- `GPU`
- `HOST`
- `NVME`
- `REMOTE`（源仓库，不计入本地驻留层级）
...
- `auto500 + 1000 representative requests`
- CLI / packaging 断裂修复
- 稳定环境下测试闭环
- 文档口径全面对齐
- 跨模型家族与跨数据集扩展
...
### 主线 A：Qwen-3B 证据链
1. 以 `auto500 + 1000 representative requests` 作为论文主实验结果。
2. 汇总既有小矩阵，用于说明 `auto` 相比 `shared/dedicated` 的定位与趋势。
...
- `28185` full trace 与 `auto1000 + full trace` 仅作为压力测试或附录接口，不是默认主线
