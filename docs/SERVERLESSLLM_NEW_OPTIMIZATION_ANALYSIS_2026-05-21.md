...
>
> `min_instances=4 + post-deploy wait 90s` 本质上把 ServerlessLLM-new 从「按需扩缩容的 serverless 系统」改成了「4 个常驻实例的 serverful 系统」。ServerlessLLM-new 的核心设计是请求驱动的弹性 scaling，冷启动和 scale-up 延迟是其 serverless 特性的固有代价。通过预创建全部实例并预热等待来消除这些代价，等于绕过而非解决 serverless 系统的核心挑战——这与 PrimeLoRA 在真实 serverless 约束下（弹性扩缩、按需加载）的优化方向完全不同。
>
> 若将 warm-min4 变体与 PrimeLoRA 放在同一张主表中对比，前者资源策略是「4 实例常驻全开」，后者是「负载驱动弹性扩缩」，两者不在同一 serverless 语义下竞争。这不公平。
>
...
2. **绕过了而非解决了问题**：系统不再需要 scale-up，也不再产生冷启动延迟。但这正是 serverless 系统最核心的挑战——warm-min4 不是「优化 serverless 性能」，而是「不做 serverless 了」。
3. **与 PrimeLoRA 对比不公平**：PrimeLoRA 在真实 serverless 约束下运行（弹性扩缩容、按需加载 adapter），ServerlessLLM-new-warm-min4 在 serverful 常驻满配下运行。两者不在同一语义下竞争，任何对比结果都不能声称是公平的 serverless 系统比较。
4. **小规模改善不代表正式负载有效**：128 请求窗口的 TTFT 改善确实证明瓶颈不在冷启动本身，而在 4000 请求稳态下的 dispatch 排队吞吐。但这只能作为诊断证据，不能作为「优化后的 baseline」——因为优化手段本身是不合理的。

**最终结论：**

- 原始 ServerlessLLM-new（不改部署策略、官方核心代码、serverless 扩缩容）是**公平的候选对比行**，性能差是其 serverless 架构在该负载下的真实表现。
- `ServerlessLLM-new-warm-min4-t32` 是**不合理的 baseline 优化**，因为它改变了系统的 serverless 语义，不应作为任何性能对比表中的数据行。它唯一的价值是作为诊断记录，证明「full-trace 瓶颈主要在稳态吞吐和排队，而不是启动期冷启动」。
- 若论文需要讨论 ServerlessLLM-new 的性能空间，应在 Related Work 或 Discussion 中以文字描述该诊断发现，而非将 warm-min4 作为正式对比行。
