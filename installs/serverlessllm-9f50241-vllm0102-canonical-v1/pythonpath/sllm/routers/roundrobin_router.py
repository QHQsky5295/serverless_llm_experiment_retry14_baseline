...
def _log_lifecycle_event(
    ...
    *,
    ...
    logger.info("SLLM_LIFECYCLE_EVENT %s", json.dumps(payload, sort_keys=True))
...
class RoundRobinRouter(SllmRouter):
    def __init__(
        ...
        self.request_poll_interval = max(
            0.001,
            float(os.getenv("SLLM_REQUEST_POLL_INTERVAL_S", "1")),
        )
    ...
    async def inference(self, request_data: dict, action: str):
        ...
        internal_metrics["m3_enqueue_monotonic_ns"] = enqueue_monotonic_ns
        ...
        try:
            ...
            internal_metrics["m3_instance_assigned_monotonic_ns"] = (
                assigned_monotonic_ns
            )
            ...
            async with self.instance_management_lock:
                ...
                internal_metrics["m3_instance_created_monotonic_ns"] = (
                    instance.created_monotonic_ns
                )
                internal_metrics["m3_runtime_ready_monotonic_ns"] = (
                    instance.ready_monotonic_ns
                )
                scaleup_affected = bool(
                    instance.ready_monotonic_ns is not None
                    and enqueue_monotonic_ns <= instance.ready_monotonic_ns
                )
                ...
                if scaleup_first_service:
                    ...
                    instance.first_request_assigned_monotonic_ns = (
                        assigned_monotonic_ns
                    )
        ...
        while True:
            ...
            while not allocated:
                ...
                while not instance_options:
                    await asyncio.sleep(self.request_poll_interval)
                    async with self.instance_management_lock:
    ...
    async def _create_instance(self):
        ...
        _log_lifecycle_event(
            ...
            instance_id=instance_id,
            timestamp_s=instance.created_at,
            monotonic_ns=instance.created_monotonic_ns,
        )
    ...
    async def _start_instance(self, instance_id):
        ...
        async with instance.lock:
            ...
            instance.ready_monotonic_ns = time.monotonic_ns()
        ...
        _log_lifecycle_event(
            ...
            model_name=self.model_name,
            instance_id=instance_id,
            timestamp_s=instance.ready_at,
            monotonic_ns=instance.ready_monotonic_ns,
        )
        logger.info(
            f"Instance {instance_id} is ready for model {self.model_name}"
        )
        return instance_id
