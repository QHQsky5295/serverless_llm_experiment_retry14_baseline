...
_M3_MONOTONIC_RESOLUTION_NS = max(
    1, int(round(time.clock_getres(time.CLOCK_MONOTONIC) * 1_000_000_000))
)
...
def create_app() -> FastAPI:
    ...
    async def inference_handler(request: Request, action: str):
        ...
        internal_metrics["request_received_at"] = time.time()
        internal_metrics["request_id"] = request_id
        internal_metrics["m3_ingress_monotonic_ns"] = ingress_monotonic_ns
        internal_metrics["m3_ingress_boot_id"] = _M3_BOOT_ID
        internal_metrics["m3_monotonic_resolution_ns"] = (
            _M3_MONOTONIC_RESOLUTION_NS
        )
        internal_metrics["m3_ingress_pid"] = os.getpid()
