...
class Batch:
    ...
    def sample(self, logits: torch.Tensor):
        ...
        for row_index, req in enumerate(self.reqs):
            target_ids = getattr(
                req.sampling_params,
            ...
            if req.regex_fsm is not None:
                raise RuntimeError(
                    "RelayServe canonical replay cannot be combined with regex"
                )
            output_index = len(req.output_ids)
            if output_index >= len(target_ids):
                raise RuntimeError("RelayServe canonical replay target exhausted")
            target_id = target_ids[output_index]
            if target_id >= logits.shape[-1]:
                raise RuntimeError("RelayServe canonical replay token is outside vocabulary")
            logits[row_index].fill_(float("-inf"))
            logits[row_index, target_id] = 0.0
