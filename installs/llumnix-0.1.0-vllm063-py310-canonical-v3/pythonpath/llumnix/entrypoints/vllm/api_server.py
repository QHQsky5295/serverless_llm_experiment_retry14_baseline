...
from relayserve.protocol_v4_3.llumnix_canonical_runtime import (
    build_llumnix_replay_processor,
)
...
from llumnix.entrypoints.utils import is_gpu_available, LlumnixTraceInfo
...
async def generate(request: Request) -> Response:
    ...
    async def stream_results() -> AsyncGenerator[bytes, None]:
        async for llumnix_request_output in results_generator.generator():
            llumnix_request_output: LlumnixRequestOuput = llumnix_request_output
...
async def generate_benchmark(request: Request) -> Response:
    ...
    if "prompt" in request_dict:
        raise ValueError("canonical Llumnix requests forbid prompt text")
    ...
    if (
        ...
        raise ValueError("invalid canonical Llumnix prompt token IDs")
    prompt = {"prompt_token_ids": list(prompt_token_ids)}
    _ = request_dict.pop("stream", False)
    request_id = request_dict.pop("relayserve_request_id")
    if not isinstance(request_id, str) or not request_id:
        raise ValueError("invalid canonical Llumnix request ID")
    replay_xargs = request_dict.pop("relayserve_canonical_replay_xargs")
    frozen_eos_token_ids = request_dict.pop(
        "relayserve_frozen_eos_token_ids"
    )
    replay_processor, replay_target = build_llumnix_replay_processor(
        replay_xargs,
        max_tokens=request_dict.get("max_tokens"),
        frozen_eos_token_ids=frozen_eos_token_ids,
    )
    if request_dict.get("logits_processors") is not None:
        raise ValueError("canonical Llumnix request supplied extra processor")
    request_dict["logits_processors"] = [replay_processor]
    sampling_params = SamplingParams(**request_dict)
    ...
    async for llumnix_request_output in results_generator.generator():
        ...
        request_processing_context = (
            llumnix_request_output.request_processing_context
        )
        if (
            not request_processing_context.enable_trace
            or request_processing_context.trace_timeline is None
        ):
            raise RuntimeError(
                "canonical Llumnix request lacks native trace timestamps"
            )
        token_id_events.append({
            "monotonic_ns": time.monotonic_ns(),
            "instance_id": llumnix_request_output.instance_id,
            "engine_step_begin_monotonic_s": (
                request_processing_context.trace_timeline.
                engine_step_timestamp_begin
            ),
            "engine_step_end_monotonic_s": (
                request_processing_context.trace_timeline.
    ...
    ret = {
        ...
        'output_token_ids': list(final_output.outputs[0].token_ids),
        'token_id_events': token_id_events,
        'finish_reason': final_output.outputs[0].finish_reason,
        'canonical_replay_applied': True,
        'canonical_target_digest': replay_target.digest,
    }
...
@app.get("/relayserve_native_state")
async def relayserve_native_state():
    return {
        "ready": await llumnix_client.is_ready(),
        "active_request_count": len(llumnix_client.request_stream),
        "tracked_request_count": len(llumnix_client.request_instances),
        "manager_available": bool(llumnix_client.manager_available),
