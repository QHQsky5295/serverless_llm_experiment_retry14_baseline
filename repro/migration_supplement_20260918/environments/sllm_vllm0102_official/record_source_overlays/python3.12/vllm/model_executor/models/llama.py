...
_DISABLE_LORA_EMBEDDINGS = (
    os.getenv("VLLM_DISABLE_LORA_EMBEDDINGS", "0").strip().lower()
    in {"1", "true", "yes", "on"}
)
...
class LlamaModel(nn.Module):
    ...
    def __init__(self,
        ...
        lora_vocab = (lora_config.lora_extra_vocab_size *
                      (lora_config.max_loras or 1)) if (
                          lora_config and not _DISABLE_LORA_EMBEDDINGS
                      ) else 0
...
class LlamaForCausalLM(nn.Module, SupportsLoRA, SupportsPP, SupportsEagle3):
    ...
    embedding_modules = (
        {}
        if _DISABLE_LORA_EMBEDDINGS
        else {
            "embed_tokens": "input_embeddings",
            "lm_head": "output_embeddings",
        }
    )
    embedding_padding_modules = [] if _DISABLE_LORA_EMBEDDINGS else ["lm_head"]
    ...
    def __init__(self,
        ...
        if get_pp_group().is_last_rank:
            ...
            if lora_config and not _DISABLE_LORA_EMBEDDINGS:
