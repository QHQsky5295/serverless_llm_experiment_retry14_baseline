...
def _parse_system_round_overrides(specs: Sequence[str]) -> dict[str, dict[str, Path]]:
    ...
    for spec in specs:
        ...
        if model_key not in MODEL_KEY_LABELS:
...
def build_lifecycle_figure(models: Sequence[ModelRound], out_dir: Path) -> None:
    ...
    for name, keys, color in components:
        vals = np.asarray([sum(system.metrics[key] for key in keys) * 1000.0 for system in systems])
...
def write_manifest(models: Sequence[ModelRound], out_dir: Path) -> None:
    payload = {
        "artifact": "main_llama_family_tables_and_lifecycle_figure",
    ...
    for name in (
        "main_llama_family_manifest.json",
...
def main() -> None:
    ...
    if args.round_1b:
        ...
        models.append(
            load_model_round(
                "llama32_3b",
                MODEL_KEY_LABELS["llama32_3b"],
                args.round_3b.resolve(),
                system_round_overrides=overrides.get("llama32_3b", {}),
                system_summary_overrides=summary_overrides.get("llama32_3b", {}),
                allow_missing_systems=allow_missing_systems,
            )
        )
    else:
