...
do
  ...
  if [[ -n "${_vllm_env_value}" ]]; then
    VLLM_ENV_PREFIX+=" ${_vllm_env_key}=$(printf '%q' "${_vllm_env_value}")"
  fi
done
unset _vllm_env_key _vllm_env_value
...
tmux new-session -d -s "${HEAD_SESSION}" -c "${ROOT_DIR}" \
  "env${VLLM_ENV_PREFIX} SLLM_HEAD_ENV=${HEAD_ENV} SLLM_DIRECT_PATH_MODE=${DIRECT_PATH_MODE} SLLM_RAY_HEAD_HOST=${RAY_HEAD_HOST} SLLM_RAY_NODE_IP=${RAY_HEAD_HOST} SLLM_RAY_ADDRESS=${RAY_ADDRESS} bash ${SCRIPTS_DIR}/run_serverlessllm_head.sh"
wait_for_head

if [[ "${SLLM_SINGLE_HOST_MULTI_GPU}" == "1" ]]; then
  tmux new-session -d -s "${WORKER_SESSION_PREFIX}_0" -c "${ROOT_DIR}" \
    "env${VLLM_ENV_PREFIX} SLLM_WORKER_ENV=${WORKER_ENV} SLLM_DIRECT_PATH_MODE=${DIRECT_PATH_MODE} SLLM_RAY_HEAD_HOST=${RAY_HEAD_HOST} SLLM_RAY_NODE_IP=${RAY_HEAD_HOST} SLLM_RAY_ADDRESS=${RAY_ADDRESS} CUDA_VISIBLE_DEVICES=${WORKER_GPUS} SLLM_WORKER_ID=0 SLLM_WORKER_NUM_GPUS=${GPU_COUNT} SLLM_WORKER_CPUS=$((GPU_COUNT * 4)) bash ${SCRIPTS_DIR}/run_serverlessllm_worker.sh"
else
  for idx in "${!GPU_LIST[@]}"; do
    gpu="${GPU_LIST[$idx]}"
    tmux new-session -d -s "${WORKER_SESSION_PREFIX}_${idx}" -c "${ROOT_DIR}" \
      "env${VLLM_ENV_PREFIX} SLLM_WORKER_ENV=${WORKER_ENV} SLLM_DIRECT_PATH_MODE=${DIRECT_PATH_MODE} SLLM_RAY_HEAD_HOST=${RAY_HEAD_HOST} SLLM_RAY_NODE_IP=${RAY_HEAD_HOST} SLLM_RAY_ADDRESS=${RAY_ADDRESS} CUDA_VISIBLE_DEVICES=${gpu} SLLM_WORKER_ID=${idx} bash ${SCRIPTS_DIR}/run_serverlessllm_worker.sh"
  done
fi
...
if [[ "${DIRECT_PATH_MODE}" != "1" ]]; then
  tmux new-session -d -s "${STORE_SESSION}" -c "${ROOT_DIR}" \
    "env${VLLM_ENV_PREFIX} SLLM_STORE_ENV=${STORE_ENV} SLLM_DIRECT_PATH_MODE=${DIRECT_PATH_MODE} CUDA_VISIBLE_DEVICES=${GPU_LIST[0]} bash ${SCRIPTS_DIR}/run_serverlessllm_store.sh"
  wait_for_store
fi

rm -f "${SERVE_LOG_PATH}"
tmux new-session -d -s "${SERVE_SESSION}" -c "${ROOT_DIR}" \
  "bash -lc 'env${VLLM_ENV_PREFIX} SLLM_HEAD_ENV=${HEAD_ENV} SLLM_DIRECT_PATH_MODE=${DIRECT_PATH_MODE} SLLM_RAY_HEAD_HOST=${RAY_HEAD_HOST} SLLM_RAY_NODE_IP=${RAY_HEAD_HOST} SLLM_RAY_ADDRESS=${RAY_ADDRESS} bash ${SCRIPTS_DIR}/run_serverlessllm_serve.sh 2>&1 | tee ${SERVE_LOG_PATH}'"
wait_for_serve
