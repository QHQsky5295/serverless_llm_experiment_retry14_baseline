#!/usr/bin/env python3
"""Replay a frozen RelayServe continuation trace through DistServe."""
...
def prepare_request(
    ...
    *,
    ...
    return {
        ...
        "request_id": str(item.get("request_id") or f"distserve-{index:06d}"),
...
def parse_token_timestamps(
    payload: dict[str, Any],
    *,
    dispatch_perf: float,
) -> tuple[float, float, float, list[int]]:
    timestamps = [float(value) for value in payload.get("timestamps") or []]
    token_ids = [int(value) for value in payload.get("token_ids") or []]
    if not timestamps:
        raise ValueError("DistServe response is missing token timestamps")
    if len(token_ids) != len(timestamps):
        raise ValueError(
            "DistServe token observability mismatch: "
            f"token_ids={len(token_ids)} timestamps={len(timestamps)}"
        )
    if any(not math.isfinite(value) for value in timestamps):
        raise ValueError("DistServe returned a non-finite token timestamp")
    if any(
        later < earlier
        for earlier, later in zip(timestamps, timestamps[1:])
    ):
        raise ValueError("DistServe token timestamps are not monotonic")

    service_ttft_ms = max(0.0, (timestamps[0] - dispatch_perf) * 1000.0)
    intervals_ms = [
        max(0.0, (later - earlier) * 1000.0)
        for earlier, later in zip(timestamps, timestamps[1:])
    ]
    service_tpot_ms = (
    ...
    service_e2e_ms = max(0.0, (timestamps[-1] - dispatch_perf) * 1000.0)
    return service_ttft_ms, service_tpot_ms, service_e2e_ms, token_ids
...
async def run(args: argparse.Namespace) -> int:
    ...
    def output_payload(*, final: bool) -> dict[str, Any]:
        return {
            ...
            "metric_definitions": {
                "primary_ttft": (
                    "scheduled trace arrival to DistServe-observed first token"
                ),
                "primary_e2e": (
                    "scheduled trace arrival to client-observed response completion"
                ),
                "service_ttft": (
                    "client dispatch to first DistServe token timestamp"
                ),
                "service_e2e": (
                    "client dispatch to last DistServe token timestamp"
                ),
                "tpot": "mean interval between DistServe token timestamps",
            },
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        async def fire(item: dict[str, Any]) -> None:
            ...
            try:
                async with session.post(endpoint, json=body) as response:
                    ...
                    if response.status != 200:
                        ...
                        if len(generated_token_ids) != item["completion_tokens"]:
                            error = (
                                "output token mismatch: "
                                f"expected={item['completion_tokens']} "
                                f"observed={len(generated_token_ids)}"
                            )
                        else:
                            success = True
            ...
            record = {
                ...
                "lifetime_events": response_payload.get("lifetime_events"),
            }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--label", default="distserve")
