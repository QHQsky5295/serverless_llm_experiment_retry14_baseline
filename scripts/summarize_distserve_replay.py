#!/usr/bin/env python3
"""Summarize a DistServe replay under the RelayServe comparison contract."""
...
def failure_reason(record: dict[str, Any]) -> str:
    ...
    return error.split(":", 1)[0][:120] if error else "unknown"
...
def main() -> int:
    ...
    validation = json.loads(
        args.semantic_validation_report.read_text(encoding="utf-8")
    )
    semantic_pass = bool(validation.get("pass"))
    ...
    strict_pass = (
        ...
        and not failed
        and all(
            len(record.get("generated_token_ids") or [])
            == int(record.get("requested_completion_tokens") or -1)
            for record in ok
        )
    )
    ttft_target = float(replay["ttft_slo_ms"])
    tpot_target = float(replay["tpot_slo_ms"])
    ttft = stats(ok, "ttft_ms")
    tpot = stats(ok, "tpot_ms")
    service_ttft = stats(ok, "service_ttft_ms")
    service_e2e = stats(ok, "service_e2e_ms")
    dispatch_wait = stats(ok, "dispatch_wait_ms")
    paper_slo_pass = (
        strict_pass
    ...
    formal_eligible = strict_pass and expected == 4000 and semantic_pass
    scenario = {
        "scenario_name": "distserve_relayserve_continuation",
        "system": "DistServe",
        "model": args.model,
        ...
        "formal_main_comparison_eligible": formal_eligible,
    ...
    payload = {
        ...
        "semantic_validation_report": str(
            args.semantic_validation_report.resolve()
        ),
        "semantic_validation_sha256": sha256(args.semantic_validation_report),
